...
BEGIN
    ...
    -- Table pour stocker les scores globaux par entreprise
CREATE TABLE IF NOT EXISTS entreprise_scores (
    id_entreprise VARCHAR(36) NOT NULL,
    score_global DECIMAL(5,2) NOT NULL,
...
-- Table pour stocker l'historique des scores globaux par entreprise
CREATE TABLE IF NOT EXISTS historique_scores_entreprises (
    id_historique VARCHAR(36) PRIMARY KEY,
    id_entreprise VARCHAR(36) NOT NULL,
    score_global DECIMAL(5,2) NOT NULL,
    date_mesure DATE NOT NULL,
...
-- Procédure stockée pour calculer le score global d'une entreprise
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS calculer_score_entreprise(IN p_id_entreprise VARCHAR(36))
BEGIN
    DECLARE v_score_global DECIMAL(5,2);
    DECLARE v_id_historique VARCHAR(36);
    ...
    -- Si un score a été calculé, l'enregistrer
    IF v_score_global IS NOT NULL THEN
        SET v_id_historique = UUID();
        
        -- Insérer ou mettre à jour le score de l'entreprise
        INSERT INTO entreprise_scores (id_entreprise, score_global, date_calcul)
        VALUES (p_id_entreprise, v_score_global, CURDATE())
...
-- Ajouter la colonne id_entreprise aux applications si elle n'existe pas
ALTER TABLE applications
ADD COLUMN IF NOT EXISTS id_entreprise VARCHAR(36) NULL,
ADD CONSTRAINT fk_application_entreprise FOREIGN KEY (id_entreprise) REFERENCES entreprises(id_entreprise);

-- Ajouter la colonne id_entreprise à la table historique_scores si elle n'existe pas
ALTER TABLE historique_scores
ADD COLUMN IF NOT EXISTS id_entreprise VARCHAR(36) NULL,
ADD CONSTRAINT fk_historique_entreprise FOREIGN KEY (id_entreprise) REFERENCES entreprises(id_entreprise);

-- Trigger pour mettre à jour les scores d'entreprise après modification des analyses
DELIMITER //
CREATE TRIGGER IF NOT EXISTS after_analysis_update
AFTER UPDATE ON maturity_analyses
FOR EACH ROW
BEGIN
    DECLARE v_id_entreprise VARCHAR(36);
    
    -- Récupérer l'ID de l'entreprise associée à l'application
    SELECT id_entreprise INTO v_id_entreprise
    FROM applications
    WHERE id_application = NEW.id_application;
    
    -- Si l'application est associée à une entreprise, recalculer le score de l'entreprise
    IF v_id_entreprise IS NOT NULL THEN
        CALL calculer_score_entreprise(v_id_entreprise);
    END IF;
END //
DELIMITER ;

-- Trigger pour mettre à jour les scores d'entreprise après insertion d'une nouvelle analyse
DELIMITER //
CREATE TRIGGER IF NOT EXISTS after_analysis_insert
AFTER INSERT ON maturity_analyses
FOR EACH ROW
BEGIN
    DECLARE v_id_entreprise VARCHAR(36);
